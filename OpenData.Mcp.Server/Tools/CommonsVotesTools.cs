...
namespace OpenData.Mcp.Server
{
    ...
    public class CommonsVotesTools(HttpClient httpClient, ILogger<CommonsVotesTools> logger, IMemoryCache cache) : BaseTools(httpClient, logger, cache)
    {
        ...
        public async Task<McpToolResponse> GetCommonsVotingRecordForMemberAsync([Description("Parliament member ID to get Commons voting record for")] int memberId)
        {
        ...
        public async Task<McpToolResponse> GetCommonsDivisionByIdAsync([Description("Unique Commons division ID number")] int divisionId)
        {
        ...
        public async Task<McpToolResponse> GetCommonsDivisionsSearchCountAsync([Description("Optional: search term to filter divisions")] string? searchTerm = null, [Description("Optional: member ID to filter divisions")] int? memberId = null, [Description("Optional: start date in YYYY-MM-DD format")] string? startDate = null, [Description("Optional: end date in YYYY-MM-DD format")] string? endDate = null, [Description("Optional: specific division number")] int? divisionNumber = null, [Description("Optional: include when member was a teller")] bool? includeWhenMemberWasTeller = null)
        {
