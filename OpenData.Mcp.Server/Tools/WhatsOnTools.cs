...
namespace OpenData.Mcp.Server
{
    ...
    public class WhatsOnTools(IHttpClientFactory httpClientFactory, ILogger<WhatsOnTools> logger, IMemoryCache cache) : BaseTools(httpClientFactory, logger, cache)
    {
        ...
        public async Task<McpToolResponse> GetSessions()
        {
        ...
        public async Task<McpToolResponse> GetNonSittingDays([Description("House name: 'Commons' or 'Lords'")] string house, [Description("Start date in YYYY-MM-DD format")] string startDate, [Description("End date in YYYY-MM-DD format")] string endDate)
        {
