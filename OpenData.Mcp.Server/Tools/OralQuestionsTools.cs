...
namespace OpenData.Mcp.Server
{
    ...
    public class OralQuestionsTools(IHttpClientFactory httpClientFactory, ILogger<OralQuestionsTools> logger, IMemoryCache cache) : BaseTools(httpClientFactory, logger, cache)
    {
        ...
        public async Task<McpToolResponse> SearchEarlyDayMotionsAsync([Description("Search term for EDM topics or content (e.g. 'climate change', 'NHS funding')")] string searchTerm)
        {
        ...
        public async Task<McpToolResponse> SearchOralQuestionTimesAsync([Description("Start date for question times in YYYY-MM-DD format")] string answeringDateStart, [Description("End date for question times in YYYY-MM-DD format")] string answeringDateEnd)
        {
